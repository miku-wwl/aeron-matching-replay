...
`ProjectionState` 是内存中的 Reference Consumer，负责顺序校验、重复检测、Digest 和
Checkpoint 状态。Replay 还提供可选的 `ReplayProjection` 扩展点：Spring 容器中注册的
Projection 会按注入顺序消费已经通过顺序校验、且刚刚成功应用的 Event；重复 Event 不会再次
