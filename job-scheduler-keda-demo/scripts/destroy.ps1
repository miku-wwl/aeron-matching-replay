...
if ($RemoveData) {
    docker compose --project-directory $demoRoot -f (Join-Path $demoRoot 'docker-compose.yml') down --volumes
    Write-Host 'Removed the demo containers and the two named demo data volumes.'
} else {
    docker compose --project-directory $demoRoot -f (Join-Path $demoRoot 'docker-compose.yml') down
